...
async function onClick(e){
  ...
  if (action === 'toggle-perseverance-mode') {
    var turningOn = !document.body.classList.contains('perseverance-mode');
    if (turningOn) {
      playPerseveranceTransition(); /* applies the mode itself once the sequence finishes */
    } else {
      applyPerseveranceMode(false); /* instant — no transition needed for turning intensity back down */
    }
    return;
  }
  if (action === 'skip-perseverance-transition') { finishPerseveranceTransition(); return; }
...
/* ==========================================================================
   Perseverance mode — transition animation
   Ported from the provided React/TSX component (KamehamehaTransition.tsx) to
   plain JS/Canvas2D — the actual drawing logic was already framework-agnostic
   (just canvas calls + requestAnimationFrame + setTimeout), only the React
   wrapper (refs, state, effects) needed translating to a plain function.
   Timing, stages, and pixel math are unchanged from the source.
   ========================================================================== */
var PERSEVERANCE_CANVAS_WIDTH = 192;
var PERSEVERANCE_CANVAS_HEIGHT = 128;
var PERSEVERANCE_ORIGIN = { x: 41, y: 50 };
var PERSEVERANCE_ORB_RADIUS = 16;
var PERSEVERANCE_MAX_DISTANCE = 292;
var PERSEVERANCE_GAP_COLOR = '#28ddff';
var PERSEVERANCE_BEAMS = [
  { angle: -38, width: 1, notch: 5 }, { angle: -9, width: 2, notch: 8 },
  { angle: 24, width: 1, notch: 3 }, { angle: 52, width: 2, notch: 7 },
  { angle: 88, width: 1, notch: 4 }, { angle: 118, width: 2, notch: 6 },
  { angle: 158, width: 1, notch: 5 }, { angle: 191, width: 2, notch: 9 },
  { angle: 222, width: 1, notch: 4 }, { angle: 258, width: 2, notch: 7 },
  { angle: 292, width: 1, notch: 6 }, { angle: 326, width: 2, notch: 5 }
];
function pClamp(v){ return Math.min(1, Math.max(0, v)); }
function pPaintPixel(ctx, x, y, color){
  if (x < 0 || y < 0 || x >= PERSEVERANCE_CANVAS_WIDTH || y >= PERSEVERANCE_CANVAS_HEIGHT) return;
  ctx.fillStyle = color;
  ctx.fillRect(x, y, 1, 1);
}
function pDrawBeam(ctx, beam, growth){
  if (growth === undefined) growth = 1;
  var angle = beam.angle * Math.PI / 180;
  var dx = Math.cos(angle), dy = Math.sin(angle);
  var px = -dy, py = dx;
  var startDistance = PERSEVERANCE_ORB_RADIUS + 1;
  var length = Math.floor(startDistance + (PERSEVERANCE_MAX_DISTANCE - startDistance) * growth);
  for (var distance = startDistance; distance < length; distance += 1) {
    var stair = Math.floor(distance / beam.notch) % 2;
    var ripple = Math.floor(distance / (beam.notch * 3)) % 2;
    for (var lane = -beam.width; lane <= beam.width; lane += 1) {
      var x = Math.round(PERSEVERANCE_ORIGIN.x + dx * distance + px * (lane + stair * 0.6));
      var y = Math.round(PERSEVERANCE_ORIGIN.y + dy * distance + py * (lane + ripple * 0.6));
      var edge = Math.abs(lane) >= beam.width;
      var color = edge ? '#047ce8' : (Math.abs(lane) === beam.width - 1 ? '#28ddff' : '#f7ffff');
      pPaintPixel(ctx, x, y, color);
    }
    if (distance > 7 && distance % (beam.notch * 2) === 0) {
      var ex = Math.round(PERSEVERANCE_ORIGIN.x + dx * distance + px * (beam.width + 1.5));
      var ey = Math.round(PERSEVERANCE_ORIGIN.y + dy * distance + py * (beam.width + 1.5));
      pPaintPixel(ctx, ex, ey, '#21dffb');
    }
  }
}
function pDrawBeamRails(ctx){ PERSEVERANCE_BEAMS.forEach(function(b){ pDrawBeam(ctx, b); }); }
function pDrawGapStrip(ctx, angleInDegrees){
  var angle = angleInDegrees * Math.PI / 180;
  var dx = Math.cos(angle), dy = Math.sin(angle);
  var px = -dy, py = dx;
  for (var distance = PERSEVERANCE_ORB_RADIUS + 1; distance < PERSEVERANCE_MAX_DISTANCE; distance += 1) {
    for (var lane = -1; lane <= 1; lane += 1) {
      var x = Math.round(PERSEVERANCE_ORIGIN.x + dx * distance + px * lane);
      var y = Math.round(PERSEVERANCE_ORIGIN.y + dy * distance + py * lane);
      pPaintPixel(ctx, x, y, PERSEVERANCE_GAP_COLOR);
    }
  }
}
function pDrawRaysAndSectors(ctx, progress){
  PERSEVERANCE_BEAMS.forEach(function(beam, index){
    var following = PERSEVERANCE_BEAMS[(index + 1) % PERSEVERANCE_BEAMS.length];
    var clockwiseEnd = following.angle <= beam.angle ? following.angle + 360 : following.angle;
    var currentEnd = beam.angle + (clockwiseEnd - beam.angle) * progress;
    for (var fillAngle = beam.angle + 0.32; fillAngle < currentEnd; fillAngle += 0.32) {
      pDrawGapStrip(ctx, fillAngle % 360);
    }
  });
  pDrawBeamRails(ctx); /* rails painted last so the original stepped rays stay crisp on top of the sector fill */
}
function pDrawPixelOrb(ctx, radius){
  var outerRadius = radius + 3;
  for (var y = -outerRadius; y <= outerRadius; y += 1) {
    for (var x = -outerRadius; x <= outerRadius; x += 1) {
      var distance = Math.hypot(x, y);
      if (distance > outerRadius) continue;
      var color = distance > radius + 1 ? '#047ce8' : distance > radius - 1 ? '#28ddff' : distance > radius - 4 ? '#c6f9ff' : '#fbffff';
      pPaintPixel(ctx, PERSEVERANCE_ORIGIN.x + x, PERSEVERANCE_ORIGIN.y + y, color);
    }
  }
}
function pDrawWhiteOvercharge(ctx, progress){
  if (progress >= 1) {
    ctx.fillStyle = '#fbffff';
    ctx.fillRect(0, 0, PERSEVERANCE_CANVAS_WIDTH, PERSEVERANCE_CANVAS_HEIGHT);
    return;
  }
  var maximumRadius = Math.hypot(PERSEVERANCE_CANVAS_WIDTH, PERSEVERANCE_CANVAS_HEIGHT);
  var acceleratedProgress = 1 - Math.pow(1 - progress, 4);
  var radius = Math.ceil(PERSEVERANCE_ORB_RADIUS + (maximumRadius - PERSEVERANCE_ORB_RADIUS) * acceleratedProgress);
  for (var y = -radius; y <= radius; y += 1) {
    for (var x = -radius; x <= radius; x += 1) {
      if (x * x + y * y <= radius * radius) pPaintPixel(ctx, PERSEVERANCE_ORIGIN.x + x, PERSEVERANCE_ORIGIN.y + y, '#fbffff');
    }
  }
}
function pDrawFrame(ctx, stage, isPlaying, elapsed){
  ctx.clearRect(0, 0, PERSEVERANCE_CANVAS_WIDTH, PERSEVERANCE_CANVAS_HEIGHT);
  if (stage === 1) { pDrawPixelOrb(ctx, 9 + (isPlaying && Math.floor(elapsed / 110) % 2 ? 1 : 0)); return; }
  if (stage === 2) { var p2 = isPlaying ? pClamp(elapsed / 300) : 1; pDrawPixelOrb(ctx, Math.round(9 + (11 - 9) * (p2 * (2 - p2)))); return; }
  if (stage === 3) { var p3 = isPlaying ? pClamp(elapsed / 650) : 1; pDrawPixelOrb(ctx, Math.round(11 + (14 - 11) * (p3 * (2 - p3)))); return; }
  var sectorProgress = isPlaying ? pClamp(0.08 + elapsed / 190) : 1;
  var overchargeProgress = isPlaying ? pClamp((elapsed - 150) / 520) : 1;
  if (overchargeProgress < 1) { pDrawRaysAndSectors(ctx, sectorProgress); pDrawWhiteOvercharge(ctx, overchargeProgress); }
  else { pDrawWhiteOvercharge(ctx, 1); }
}

var perseveranceAnimFrame = null;
var perseveranceTimers = [];
function perseveranceClearTimers(){
  perseveranceTimers.forEach(function(t){ window.clearTimeout(t); });
  perseveranceTimers = [];
  if (perseveranceAnimFrame) { window.cancelAnimationFrame(perseveranceAnimFrame); perseveranceAnimFrame = null; }
}
/* Applies the theme immediately, no transition — used for the OFF direction
   (an instant, unremarkable toggle back is the right feel there) and as the
   final step once the ON-direction transition finishes or is skipped. */
function applyPerseveranceMode(on){
  document.body.classList.toggle('perseverance-mode', on);
  try { localStorage.setItem('practex_perseverance_mode', on ? '1' : '0'); } catch(e) {}
  renderPerseveranceBadge();
  if (typeof render === 'function' && typeof state !== 'undefined' && state.mcqs) render(); /* refreshes the Settings modal's own toggle switch if it's open — same hook every other toggle already relies on (render() re-renders the modal content when state.activeModal === 'settings') */
}
function finishPerseveranceTransition(){
  perseveranceClearTimers();
  var overlay = document.getElementById('perseveranceOverlay');
  if (overlay) overlay.classList.remove('is-active', 'is-charging', 'is-whiteout');
  applyPerseveranceMode(true);
}
function playPerseveranceTransition(){
  perseveranceClearTimers();
  var overlay = document.getElementById('perseveranceOverlay');
  var canvas = document.getElementById('perseveranceCanvas');
  if (!overlay || !canvas) { applyPerseveranceMode(true); return; } /* no DOM to animate against — just apply the mode directly rather than getting stuck */
  ...
  function render(now){
    ...
    pDrawFrame(ctx, stage, playing, elapsed);
    if (playing) perseveranceAnimFrame = window.requestAnimationFrame(render);
  }
  perseveranceAnimFrame = window.requestAnimationFrame(render);

  function cue(delay, nextStage, extraClass){
    perseveranceTimers.push(window.setTimeout(function(){
      stage = nextStage;
  ...
  cue(620, 2);
  cue(920, 3);
  cue(1570, 5, 'is-charging');
  perseveranceTimers.push(window.setTimeout(function(){ overlay.classList.add('is-whiteout'); }, 1570 + 520));
  perseveranceTimers.push(window.setTimeout(function(){
    playing = false;
    finishPerseveranceTransition();
  }, 2520));
}

/* Flame-hoarding badge — persistent top-right indicator while the mode is
   active, separate from the one-time transition above. */
function renderPerseveranceBadge(){
  var root = document.getElementById('perseveranceBadgeRoot');
  if (!root) return;
  if (!document.body.classList.contains('perseverance-mode')) { root.innerHTML = ''; return; }
  root.innerHTML = '<div class="perseverance-badge" title="Perseverance mode is on — click to turn off" data-action="toggle-perseverance-mode">' +
    '<span class="perseverance-badge-flames"><span class="perseverance-badge-flame"></span><span class="perseverance-badge-flame"></span><span class="perseverance-badge-flame"></span></span>' +
    '<span class="pb-label">PERSEVERANCE</span></div>';
}

async function init(){
  document.body.addEventListener('click', onClick); /* bound once — delegates for both #appRoot, #authGate, and #modalRoot */
  try { if (localStorage.getItem('practex_perseverance_mode') === '1') { document.body.classList.add('perseverance-mode'); renderPerseveranceBadge(); } } catch(e) {}
